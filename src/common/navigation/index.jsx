...
const Nav = () => {
    ...
    return (
        <div ref={ref}>
            <nav className='w-3/4 border-r-primaryLighter border-r shadow-light px-4 pb-8 fixed top-0 bg-white' id='nav' >
                <FaTimes 
                ...
                <ul>
                    {
                        navItems.map(({ icon, label, link }) => <Link key={label} to={link} className='flex w-full gap-4 text-primary text-sm font-medium py-2 px-4 hover:bg-sidebar-hover hover:shadow-light rounded-sm nav-item'>
                            {icon}
                            <span>{label}</span>
                        </Link>)
                    }
